...
IGL_INLINE void igl::circulation(
    ...
    /*std::vector<int> & Ne,*/
    std::vector<int>& Nv,
    std::vector<int>& Nf, const Eigen::MatrixXi& E, Eigen::MatrixXd& V, igl::min_heap< std::tuple<double, int, int> >& Q,
    Eigen::MatrixXd& C)
{
    /*
    our :
    in this function we are mapping the edges of a selected edge that we got from
    the Qheap, as we can see that bf is the selected edge therfor the edges that touches
    the verteces of this edge (b and f) will be mapped and the common triangles
    we will get from this action will be removed.
    ...
    */
        ...
        int& nf)
    {
        ...
        rv = F(nf, nv);
        ne = EMAP(nf + m * ((nv + dir + 3) % 3));
        std::vector<double > VYossi;
        std::vector<double > VDani;
